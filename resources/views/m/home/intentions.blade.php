@include('m.public.header')
<div class="clear"></div>
<div class="main02">
    @include('m.home.editUser')
   <div class="grzy_wdxx_meun">
        <span>
            <a href="{{ route('home.index') }}">消息</a>
            <a  href="{{ route('home.like_colleges') }}" >收藏</a>
            <a id="grzy_wdxx_meun" href="{{ route('home.intentions') }}" >意向单</a>
        </span>
    </div>

    <div class="pinggu_xx" >
        <div class="pinggu_xx_name">
            <img src="/static/images/xxm.jpg">
            <h1>悉尼大学<br>The University of Sydney</h1>
            <div class="clear"></div>
        </div>
        <div class="pinggu_pm01">
            <em>专业</em>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>雅思<br>80</span>
            <em>专业工作</em>
            <div class="clear"></div>
        </div>
    </div>
    <div class="pinggu_pm02">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>
    <div class="pinggu_pm03">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>


    <div class="pinggu_xx" >
        <div class="pinggu_xx_name">
            <img src="/static/images/xxm.jpg">
            <h1>悉尼大学<br>The University of Sydney</h1>
            <div class="clear"></div>
        </div>
        <div class="pinggu_pm01">
            <em>专业</em>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>雅思<br>80</span>
            <em>专业工作</em>
            <div class="clear"></div>
        </div>
    </div>
    <div class="pinggu_pm02">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>
    <div class="pinggu_pm03">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>


    <div class="pinggu_xx">
        <div class="pinggu_xx_name">
            <img src="/static/images/xxm.jpg">
            <h1>悉尼大学<br>The University of Sydney</h1>
            <div class="clear"></div>
        </div>
        <div class="pinggu_pm01">
            <em>专业</em>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>雅思<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>U.S.New排名<br>80</span>
            <span>雅思<br>80</span>
            <em>专业工作</em>
            <div class="clear"></div>
        </div>
    </div>
    <div class="pinggu_pm02">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>
    <div class="pinggu_pm03">
        <em>专业</em>
        <span>雅思</span>
        <span>雅思</span>
        <span>雅思</span>
        <span>U.S.New排名</span>
        <span>U.S.New排名</span>
        <span>雅思</span>
        <em>专业工作</em>
        <a href="#"><img src="/static/images/icon18.png" width="18" height="18"></a>
        <div class="clear"></div>
    </div>
    <div class="clear"></div>
</div>
